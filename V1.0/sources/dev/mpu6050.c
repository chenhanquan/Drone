...
static float FindPos(float*a,int low,int high);    //找下标函数
static void QuiteSort(float* a,int low,int high);  //快速排序函数
...
static void MPU6050_ByteWrite(u8 slaveAddr,u8 writeAddr ,u8* pBuffer);                           //写一位寄存器
...
static void SortAver_FilterXYZ(MPU6050Data_TypeDef *MPU6050Data);                                //加速度去极值滤波
...
void MPU6050_Init()
{ 
	MPU6050_ByteWrite(MPU6050_DEFAULT_ADDRESS,MPU6050_RA_PWR_MGMT_1, (u8*)0x01);      //时钟选择
	MPU6050_ByteWrite(MPU6050_DEFAULT_ADDRESS,MPU6050_RA_SMPLRT_DIV, (u8*)0x00);      // sample rate.  Fsample= 1Khz/(<this value>+1) = 1000Hz
//	MPU6050_ByteWrite(MPU6050_DEFAULT_ADDRESS,MPU6050_RA_CONFIG, (u8*)0x02);          //内部低通滤波频率，98hz
	MPU6050_ByteWrite(MPU6050_DEFAULT_ADDRESS,MPU6050_RA_GYRO_CONFIG, (u8*)0x10);     //1000deg/s
	MPU6050_ByteWrite(MPU6050_DEFAULT_ADDRESS,MPU6050_RA_ACCEL_CONFIG, (u8*)0x10);    //加速度 正负8g
}
