	...
	for(u8 i=0;i<TASK_NUM;i++)                //轮询任务
 	{
		...
		switch(i)
		{
			...
			case 1:
			{
				double temp[3]={0};
				vs16 data_temp;
				temp[0]=KalmanData.roll*1.7452;    //100/57.2 发送的为弧度数据
				temp[1]=KalmanData.pitch*1.7452;
				temp[2]=KalmanData.yaw*1.7452;
...
//				vSendOneFrame(DRONE_ADDR, SEND_ID_QUAD,9,uDataToSend);
			...
			case 3:
			{
				float temp[3]={0};
				vs16 temp1;
				for(u8 i=0;i<3;i++){temp[i]=CompassData.mag[i];}
				for(u8 i=0;i<3;i++)
				{
					temp1=(s16)temp[i];
					uDataToSend[2*i]=BYTE1(temp1);
				...
				for(u8 i=0;i<11;i++){uDataToSend[i+3]=0;}
